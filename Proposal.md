...
HA operating system powers the HA Server program, which is the backbone of the HA system. HA is controlled by using Supervisor which manages Core which includes both pre-installed and optional add-on applications. Core also comes in a Docker container version.
